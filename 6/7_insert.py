...
print("\nActivation Nodes:")
draw_graph(traced_with_activation, "./before.svg")
for node in activation_nodes:
    with traced_with_activation.graph.inserting_after(node):
        new_node=traced_with_activation.graph.call_method("parameter_offload",(node,),dict(),None)
    print(node)
draw_graph(traced_with_activation, "./insert_before.svg")
for node in activation_nodes:
    with traced_with_activation.graph.inserting_after(node):
        new_node=traced_with_activation.graph.call_method("parameter_offload",(node,),dict(),None)
    print(node)
draw_graph(traced_with_activation, "./insert_after.svg")